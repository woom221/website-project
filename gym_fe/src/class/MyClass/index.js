...
export default function MyClass() {
    const [pastClassesUser, setPastClassesUser] = React.useState([]);
    ...
    React.useEffect(() => {
        fetch(`http://localhost:8000/classes/view/${ localStorage.getItem("userID") }/?page=${ pageUser + 1 }`, {method: 'GET', headers: {'Authorization': "Token " + localStorage.getItem("token")}})
            .then(response => response.ok ? response.json() : window.location.href = '/login')
            .then((json) => {
                setPastClassesUser(json.results.filter(result => (new Date(result.time) < new Date())))
                setTotalPage(json.next === null || json.results.filter(result => (new Date(result.time) < new Date())).length < 10 ? pageUser + 1 : totalPage)
            }
        )}, [pageUser]);
        ...
        return (
            ...
            <TableContainer component={Paper} sx={{maxHeight: "750px", width: '70%', overflowX: 'scroll', overflowY: 'scroll'}}>
                <Table sx={{ width: '100%' }}>
                    ...
                    <TableBody>
                        <TableRow>
                            <TableCell align="center" sx={{ padding: "10px 10px 10px 10px"}}>{ pastClassesUser.map((each_class) =>
